...
OUTPUT_XLSX = BASE_DIR / "lms_calendar.xlsx"  # 기존 단독 저장용 이름: 통합 저장에서는 사용하지 않음
OUTPUT_CSV = BASE_DIR / "lms_calendar.csv"
INTEGRATED_EXCEL_PATH = BASE_DIR / "KACE_통합관리 통합.xlsx"
USERFORM_XLSM_PATH = BASE_DIR / "userform.xlsm"
...
def load_lms_settings_from_userform(userform_path):
    """
    userform.xlsm의 '사용자설정' 시트에서 LMS 우선순위 설정을 읽는다.

    시트 구조:
    - A:D = 유형 / 사용 여부 / 유형 가중치 / 검색 키워드
    - F:I = 과목 키워드 / 사용 여부 / 과목 가중치 / 메모
    """
    try:
        from openpyxl import load_workbook
    except ImportError:
        print("openpyxl이 없어 사용자설정 시트를 읽지 못했습니다. 기본 설정을 사용합니다.")
        return {}

    userform_path = Path(userform_path)
    if not userform_path.exists():
        print(f"사용자설정 파일을 찾지 못했습니다: {userform_path}")
        return {}

    wb = load_workbook(userform_path, data_only=True, keep_vba=True)
    if "사용자설정" not in wb.sheetnames:
        print("userform.xlsm에 '사용자설정' 시트가 없습니다. 기본 설정을 사용합니다.")
        return {}
    ...
    for row in range(5, ws.max_row + 1):
        ...
        type_settings.append({
            ...
            "enabled": ws.cell(row=row, column=2).value or "N",
            "weight": ws.cell(row=row, column=3).value or 1,
            "keywords": ws.cell(row=row, column=4).value or "",
        })

    course_settings = []
    for row in range(5, ws.max_row + 1):
        course = ws.cell(row=row, column=6).value
        if course is None or str(course).strip() == "":
            continue

        course_settings.append({
            "course": str(course).strip(),
            "enabled": ws.cell(row=row, column=7).value or "N",
            "weight": ws.cell(row=row, column=8).value or 1,
            "memo": ws.cell(row=row, column=9).value or "",
        })

    print(f"사용자설정 로드 완료: 유형 {len(type_settings)}개, 과목 {len(course_settings)}개")
    return {
...
def clear_rows_below_header(ws):
    """헤더 1행은 유지하고 기존 데이터만 삭제"""
    if ws.max_row > 1:
        ws.delete_rows(2, ws.max_row - 1)


def ensure_sheet_headers(wb, sheet_name, headers):
...
def apply_basic_style(ws, widths):
    from openpyxl.styles import Font, PatternFill, Alignment
...
def save_to_result_workbook(calendar_events, announcements, result_path, settings=None):
    """
    통합 엑셀이 열려 있어도 충돌이 나지 않도록,
    LMS 결과를 임시 결과 파일(result_path)에 저장한다.
    이후 VBA가 이 결과 파일의 시트를 현재 열려 있는 통합 엑셀로 복사한다.
    """
    try:
    ...
    apply_basic_style(ws1, {"A": 14, "B": 45, "C": 18, "D": 80, "E": 60})
    ...
    for col_idx, header in enumerate(ann_headers, start=1):
    ...
    apply_basic_style(ws2, {"A": 45, "B": 90, "C": 22})

    # =========================
    # 3. 우선순위 정렬 시트
    #    userform.xlsm의 사용자설정 값을 반영한 결과
    # =========================
    ...
    apply_basic_style(ws3, {
        "A": 10, "B": 12, "C": 14, "D": 28,
        "E": 90, "F": 22, "G": 35, "H": 60
    })

    wb.save(result_path)
    print(f"LMS 결과 파일 저장 완료: {result_path}")
    print(f"LMS 일정 저장 건수: {len(calendar_events)}개")
    print(f"공지사항 저장 건수: {len(announcements)}개")
    print(f"우선순위 정렬 저장 건수: {len(priority_rows)}개")
...
def main():
    ...
    # VBA가 settings를 직접 넘기면 그 값을 쓰고, 없으면 userform.xlsm의 '사용자설정' 시트에서 읽는다.
    settings = data.get("settings") or load_lms_settings_from_userform(
        data.get("userform_path", USERFORM_XLSM_PATH)
    )
    result_path = Path(data.get("result_path", OUTPUT_XLSX))
    ...
    try:
        ...
        # 6. 임시 결과 엑셀 저장
        save_to_result_workbook(calendar_events, announcements, result_path, settings)

        print("완료!")
        print(f"캘린더 일정: {len(calendar_events)}개")
        print(f"공지사항: {len(announcements)}개")
        print(f"결과 파일 저장 위치: {result_path}")
